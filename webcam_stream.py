...
def log_system_event(subsystem: str, message: str, level: str = "INFO"):
    """全域安全日誌器：同時寫入硬碟並透過 WebSocket 即時向網頁對話框發送事件"""
...
async def lifespan(app: FastAPI):
    global main_loop
    main_loop = asyncio.get_running_loop()  # 🌟 鎖定 Uvicorn 主事件循環
...
async def handle_login(request: Request): #
    ...
    remember = form_data.get("remember")  # 接收勾選框狀態
