...
class VideoCamera:
    def __init__(self, cam_id, source, shared_model=None):
        """
        初始化個別相機頻道
        :param cam_id: 相機識別代號 (如 'cam_0')
        :param source: 訊號源 (如 0, 1, 2 代表 USB 鏡頭)
        """
        self.cam_id = cam_id
        self.source = source
    ...
    def get_frame_generator(self, mode="smooth"):
        """🌟 智慧安防雙碼流分發器：達成肉眼可見的完美畫質/解析度切換"""
        while self.is_running:
            if self.Frame is None:
                time.sleep(0.05) #
                continue #
                
            # 🌟 核心升級：實做真正的解析度尺寸與品質分流
            if mode == "high":
                # 💎 完整高畫質：直接輸出實體相機的 100% 原始尺寸，JPEG 壓縮比調至最高品質 95
                out_frame = self.Frame.copy()
                encode_quality = 95
                delay = 0.03  # 鎖定高效流暢高刷
            else:
                # ⚡ 輕量化流暢：自動將畫面等比例縮小（寬度降至 640），並將品質調為 50，大幅度消滅延遲
                ...
                if scale < 1.0:
                    ...
                    out_frame = self.Frame.copy()
                encode_quality = 50
                delay = 0.02  # 更低的睡眠延時，追求絲滑超流暢

            # 將內部的 BGR 矩陣編碼為動態轉碼後的 .jpg 二進位流
